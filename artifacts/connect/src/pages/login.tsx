...
function mapLoginError(msg: string): string {
  ...
  if (msg.includes("Username not found")) return msg;
  if (msg.includes("too-many-requests")) return "Too many attempts. Please wait a moment.";
  if (msg.includes("network-request-failed") || msg.includes("offline")) return "No internet connection.";
  if (msg.includes("user-disabled")) return "This account has been disabled.";
  if (msg.includes("Account linked")) return msg;
  return msg;
}
...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const user = await loginWithUsernameOrEmail(identifier.trim(), password);
      ...
      <div className="flex-1 flex flex-col pt-10 max-w-sm mx-auto w-full">
        ...
        <div className="relative mb-6">
          ...
          <div className="relative flex justify-center text-xs uppercase">
            <span className="bg-background px-3 text-muted-foreground">or username / email</span>
          </div>
        </div>

        <form onSubmit={handleLogin} className="space-y-4 flex-1">
          <div className="space-y-1.5">
            <Label className="text-muted-foreground text-xs uppercase tracking-wide ml-1">Username or Email</Label>
            <Input
          ...
          <Button
            type="submit"
            disabled={!identifier.trim() || !password || loading || googleLoading}
